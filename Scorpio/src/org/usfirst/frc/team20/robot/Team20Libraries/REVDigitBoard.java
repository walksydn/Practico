...
public class REVDigitBoard {
	 ...
	 * DOCUMENTATION::
	 * 
	 * REVDigitBoard() : constructor void display(String str) : displays the
	 * first four characters of the string (only alpha (converted to uppercase),
	 * numbers, and spaces) void display(double batt) : displays a decimal
	 * number (like battery voltage) in the form of 12.34
	 * (ten-one-decimal-tenth-hundredth) void clear() : clears the display
	 * boolean getButtonA() : button A on the board boolean getButtonB() :
	 * button B on the board double getPot() : potentiometer value
	 */
	...
	public void display(String str) { // only displays first 4 chars
		...
		str = repeat(' ', Math.max(0, 4 - str.length())) + str.toUpperCase();
	...
	public void display(double batt) { // optimized for battery voltage, needs a
										// double
		// like 12.34
		int[] charz = { 0, 0, 0, 0 };
	...
	public void display(int auto) {
		// like 12.34
		int[] charz = { 0, 0, 0, 0 };
		int thousand = (auto / 10);
		int hundred = (auto % 10);
		int ten = ((auto * 10) % 10);
		int one = ((auto * 100) % 10);
