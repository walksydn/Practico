 ...
 * @author Team 20
 * 
 *         Extends CANTalon to provide simple use of CAN based motor controller
 *         where position control using potentiometer feedback to controller.
 * 
 */
public class T20ServoEncPosition extends CANTalon {
	 ...
	 */
	public T20ServoEncPosition(int masterId, int[] slaves, double p, double i, double d, double zero, double span,
			double deadBand) {
		super(masterId);
		this.enabled = true;
		this.disableControl();
	...
	public void setXEU(double setPoint) {
		SmartDashboard.putString("Setpoint EU", String.valueOf(setPoint));
	...
	public final void set(double x) {
		...
		// String.valueOf(this.requestedX));
		// SmartDashboard.putString("elevator setpoint Actual",
		...
		if (this.enabled && Math.abs(this.position - this.requestedX) < this.deadBand) {
			this.disableControl();
			return;
		}
	...
	public void home() {
		...
		if (super.isFwdLimitSwitchClosed() || super.isRevLimitSwitchClosed()
				|| this.totalOutputCurrent > this.homeCurrent) {
			this.homed = true;
	...
	private void sumCurrent() {
		...
		this.talonCurrFil[this.slaves.length] = this.talonCurrFil[this.slaves.length] * .9
				+ this.getOutputCurrent() * .1;
	...
	public String toString() {
		...
		buffer = buffer + "\n P,I,D = " + this.getP() + ", " + this.getI() + ", " + this.getD();
